...
### Added
- 新增 `scrapers/sexinsex_scraper.py`:sexinsex.net 论坛列表爬虫(Discuz 7.x)。支持三种入口任选其一:`--tag <中文名>`(自动 GBK URL 编码)、`--forum <版块 id>`、`--url <任意列表 URL>`,带 `--start/--end/--pages` 翻页 + `--append` 追加去重。**不需要 Playwright**(站点匿名直通,无 Cloudflare 验证),纯 requests + BeautifulSoup。修了一个隐蔽坑:页面响应实际是 GBK 但 `<meta charset>` 错误声明为 UTF-8,脚本强制 `r.encoding='gbk'`;同时翻页时**不能走 `urlparse + parse_qs + urlencode`** 链路,因为它会把 GBK 编码的 `%CB%D8%C8%CB` 误按 UTF-8 解码成 4 个 U+FFFD 替换符,改成纯字符串正则替换 `page=N` 段。每条产出含 `title/code(tid)/url/source` 主字段 + `extra.{forum_id/forum_name/author/author_uid/posted_at/replies/views/last_reply_at/last_reply_by/tag/page}`,实测一页 50 条字段 100% 命中。⚠️ **scraper 只抓列表元数据**,帖子正文/附件/网盘链接在详情页需要登录 + 金币门槛(forum-25 亚洲无码区需金币 ≥49),后续 downloader 需配 cookie;但 forum-351 等"新手试贴/不加金币"版块匿名可下,downloader 阶段可分级处理。
- 新增 `scrapers/tg_downloader.py`:Telegram 频道资源下载器(基于 Telethon / MTProto)。支持公开频道(`@username`)、私有邀请链接(`https://t.me/+xxx` 或 `joinchat/xxx`)、数字 channel id。一次跑下来:解析频道 → 拉历史消息 → 下载所有附件(照片/视频/语音/音频/任意文件),输出 `data/tg/{slug}_{id}/{info.json, state.json, messages.jsonl, media/}`。**首次跑全量、后续靠 `state.json` 的 `last_message_id` 自动增量**,断点续传不重复;遇到 FloodWait 自动 sleep 重试。私有频道需要账号事先已加入(脚本只 `CheckChatInviteRequest` 解析,不自动加,避免误入)。凭证从环境变量 `TG_API_ID`/`TG_API_HASH` 读(或 CLI `--api-id`/`--api-hash`),session 文件存到 `~/.ziyuanku/tg/{label}.session`,**不进仓库**。配套 `scrapers/requirements_tg.txt`(只装 `telethon`),`.gitignore` 加 `*.session` / `*.session-journal` 双保险。
