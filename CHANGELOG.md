...
### Changed
- 后台改为「左侧边栏 + 内容区」专业管理后台布局：左侧深色固定侧栏按「总览 / 资源 / 分类管理 / 系统」分组归类导航(带图标与高亮态)，顶部内容栏显示当前页标题与 API 文档入口，内容区沿用升级后的卡片/表格组件，并新增移动端侧栏抽屉(汉堡按钮 + 遮罩)；样式缓存版本 `style.css?v=16`。
- 后台界面整体视觉升级（更专业的设计风格）：引入设计变量(配色/圆角/阴影)，统计卡、筛选区、工具栏、表格(粘性表头+行悬停)、状态徽章(带状态圆点)、按钮与输入框(聚焦高亮)统一打磨。
